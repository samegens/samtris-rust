...
pub struct HighScoreManager<R: HighScoresRepository> {
    repository: R,
    high_scores: HighScores,
}

impl<R: HighScoresRepository> HighScoreManager<R> {
    pub fn new(repository: R) -> Result<Self, String> {
        let high_scores = repository.load()?;
        Ok(Self {
            repository,
            high_scores,
        })
...
mod tests {
    ...
    fn new_loads_high_scores_from_repository() {
        ...
        let repository = MockHighScoresRepository::new(initial_scores);
        ...
        assert_eq!(manager.get_high_scores().len(), 1);
    }

    #[test]
    fn is_high_score_delegates_to_high_scores() {
        // Arrange
        let repository = MockHighScoresRepository::empty();
        let sut = HighScoreManager::new(repository).unwrap();
    ...
    fn add_high_score_saves_when_score_qualifies() {
        // Arrange
        let repository = MockHighScoresRepository::empty();
        let mut sut = HighScoreManager::new(repository).unwrap();
